      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <div className="space-y-2 text-sm">
            <div>
              <span className="text-cp-neutral-500">Name:</span> {submission.formData.name}
            </div>
            <div>
              <span className="text-cp-neutral-500">Email:</span> {submission.formData.email}
            </div>
            <div>
              <span className="text-cp-neutral-500">Country:</span> {submission.formData.country}
            </div>
          </div>
          ...
          <div className="space-y-2 text-sm">
            <div>
              <span className="text-cp-neutral-500">Package Type:</span> <span className="capitalize">{submission.tier}</span>
            </div>
            <div>
              <span className="text-cp-neutral-500">Credit Amount:</span> {submission.tokenAmount.toLocaleString()}
            </div>
            <div>
              <span className="text-cp-neutral-500">Submission Date:</span> {new Date(submission.timestamp).toLocaleDateString()}
            </div>
            <div>
              <span className="text-cp-neutral-500">Status:</span> {getStatusBadge(submission.status)}
            </div>
          </div>
        ...
        <div>
          ...
          <div className="space-y-2 text-sm">
            <div>
              <span className="text-cp-neutral-500">Arrival City:</span> {submission.formData.arrival}
            </div>
            <div>
          ...
          <div className="bg-white p-3 rounded-md border min-h-[80px] text-sm">
            {submission.adminNotes || <span className="text-cp-neutral-400 italic">No notes added</span>}
          </div>
